...
recipe_steps = {'step01':{'bosch':12,'iso':100,'cycles':4},
                'step02':{'bosch':None,'iso':600,'cycles':1}}
...
for par in all_param_combos:
    ...
    plot_state_dir = master_dir + 'v12_0serr_states_'+ im_file.split('.png')[0]+str(horiz_to_vert_rate_ratio)+"_"+str(alpha_f)+"_"+str(cell_size)+"_"+str(C_etch)+"/"
    if not os.path.exists(plot_state_dir):
        os.makedirs(plot_state_dir)
    plot_z_dir = master_dir + 'v12_0serr_z_'+im_file.split('.png')[0]+str(horiz_to_vert_rate_ratio)+"_"+str(alpha_f)+"_"+str(cell_size)+"_"+str(C_etch)+"/"
    if not os.path.exists(plot_z_dir):
        os.makedirs(plot_z_dir)
    
    pixel_um_conv = 212/100  # mask_WXC7R5
    #pixel_um_conv = 236/148 #w7r1c8
    ...
    save_states = {step: np.zeros_like(states) for i,step in
                   enumerate(loop_steps) if i%
                   int(len(loop_steps)/10)==0}
    save_phis = {step: np.zeros_like(phis) for i,step in
                 enumerate(loop_steps) if i%
                 int(len(loop_steps)/10)==0}
    ...
    # dict_file = plot_png_dir + 'exposed_cells_maskX_R5_C3.txt'
